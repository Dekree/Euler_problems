...
function mulMatrix() {
	...
	for( var i = 0; i < a[0].length; i++ ) {
		c[i][x] = a[i][x] * b[x][x] + a[i][x + 1] * b[x + 1][x] + a[i][x + 2] * b[x + 2][x];
		c[i][x + 1] = a[i][x] * b[x][x + 1] + a[i][x + 1] * b[x + 1][x + 1] + a[i][x + 2] * b[x + 2][x + 1];
		c[i][x + 2] = a[i][x] * b[x][x + 2] + a[i][x + 1] * b[x + 1][x + 2] + a[i][x + 2] * b[x + 2][x + 2];
...
function fibSum() {
	var sum = 0;
	for(var i = 2; i < 50; i++) {
		var num = Math.round( ( Math.pow( (1 + Math.sqrt(5)), i ) - Math.pow( (1 - Math.sqrt(5)), i ) ) / ( Math.pow(2, i) * Math.sqrt(5) ) );
		if( num % 2 == 0 ) {
			if( num >= 4000000 ) {
				return sum;
			}
...
function smalest() {
	...
	for( var i = 21; i < 1000000000; i++ ) {
		...
		if( succ.length >= 19 ) console.log( succ.length + ' при числе ' + i );
		succ = [];
	}
}
//console.log( smalest() );
...
function sqr() {
	...
	sqrSum *= sqrSum;
	if( sqrSum > sumSqr ) return sqrSum - sumSqr;
	else return sumSqr - sqrSum;
}
//console.log( sqr() );
...
function prime() {
	...
	for(var i = 2; i; i++) {
		if( i == 2 || i == 3 || i == 5 || i == 7 || ( i % 2 != 0 && i % 3 != 0 && i % 5 != 0 && i % 7 != 0 ) ) {
			ch = i;
			prostoe.push(ch);
			for( var j = 0; j < prostoe.length; j++ ) {
				if( ch % prostoe[j - 1] != 0 && prostoe[j - 1] != undefined ) {
					check++;
				}
			}
			if( check >= prostoe.length - 1 ) {
				num.push(ch);
				check = 0;
				if( num.length >= 10001 ) return num[10000];
			}
			check = 0;
		}
...
function product4() {
	...
	var num = 1;
	var str;
	var number = '7316717653133062491922511967442657474235534919493496983520312774506326239578318016984801869478851843' +
	...
	for( var i = 0; i < number.length - 12; i++ ) {
		str = number.substr(i, 13);
...
function prime2() {
	...
	for(var i = 2; i < 2000000; i++) {
		if( i == 2 || i == 3 || i == 5 || i == 7 || ( i % 2 != 0 && i % 3 != 0 && i % 5 != 0 && i % 7 != 0 ) ) {
			ch = i;
			prostoe.push(ch);
			for( var j = 2; j < Math.sqrt(ch); j++ ) {
				if( ch % j != 0 ) {
					check++;
				}
			}
			if( check >= Math.floor(Math.sqrt(ch)) - 1 ) {
				num.push(ch);
				check = 0;
			}
			check = 0;
...
function large() {
	...
	var max = 0;
	var points = [];
	var arr = [ [ 8,  2, 22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8],
			    ...
			    [85, 56,  0, 48, 35, 71, 89,  7,  5, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58],
	...
	for( var i = 0; i < arr.length; i++ ) {
		for( var j = 0; j < arr.length - 3; j++ ) {
			...
			if( num >= max ) {
				...
				max = num;
				points.push(arr[i][j]);
				points.push(arr[i][j + 1]);
				points.push(arr[i][j + 2]);
				points.push(arr[i][j + 3]);
			}
		}
		for( var k = 0; k < arr.length - 3; k++ ) {
			num = arr[k][i] * arr[k + 1][i] * arr[k + 2][i] * arr[k + 3][i];
			//console.log( arr[k][i], arr[k + 1][i], arr[k + 2][i], arr[k + 3][i], ' = ' + num );
			if( num >= max ) {
				points = [];
				max = num;
				points.push(arr[k][i]);
				points.push(arr[k + 1][i]);
				points.push(arr[k + 2][i]);
				points.push(arr[k + 3][i]);
			}
		}
		for( var x = 0; x < arr.length - 3; x++ ) {
			if( i < arr.length - 3 ) {
				num = arr[x][i] * arr[x + 1][i + 1] * arr[x + 2][i + 2] * arr[x + 3][i + 3];
				//console.log( arr[x][i], arr[x + 1][i + 1], arr[x + 2][i + 2], arr[x + 3][i + 3], ' = ' + num );
				if( num >= max ) {
					points = [];
					max = num;
					points.push(arr[x][i]);
					points.push(arr[x + 1][i + 1]);
					points.push(arr[x + 2][i + 2]);
					points.push(arr[x + 3][i + 3]);
				}
			}
		}
		for( var y = arr.length -1; y >= 3; y-- ) {
			if( i < arr.length - 3 ) {
				num = arr[i][y] * arr[i + 1][y - 1] * arr[i + 2][y - 2] * arr[i + 3][y - 3];
				//console.log( arr[i][y], arr[i + 1][y - 1], arr[i + 2][y - 2], arr[i + 3][y - 3], ' = ' + num );
				if( num >= max ) {
					points = [];
					max = num;
					points.push(arr[i][y]);
					points.push(arr[i + 1][y - 1]);
					points.push(arr[i + 2][y - 2]);
					points.push(arr[i + 3][y - 3]);
				}
			}
		}
	}
...
function triangle() {
	...
	for( var i = 5000; i; i++ ) {
		num = 0.5 * i * (i + 1);
		for( var j = 1; j <= Math.round( Math.sqrt(num) ); j++ ) {
			if( num % j == 0 ) {
				del.push(j);
			}
		}
...
function largeSum() {
	var chislo = [ '37107287533902102798797998220837590246510135740250',
...
function words() {
	...
	for( var x = 0; x < 9; x++ ) {
		...
		for(var k = 0; k < 19; k++) {
			num += ( arr[x].length + arr[27].length + arr[29].length + arr[k].length );
		}
		for(var b = 19; b < 27; b++) {
	...
	console.log( num );
	num += ( arr[0].length + arr[28].length );
	return num;
}
//console.log( words() );
...
function fak() {
	...
	var total = 0;
	for( var i = 1; i <= 100; i++ ) {
		num = num.multiply( sum.multiply(i) );
...
function abundantSum() {
	...
	console.log( abun.length );
	for( var i = 0; i < abun.length / 2; i++ ) {
		for( var y = 0; y < abun.length; y++ ) {
			if( num.indexOf( abun[i] + abun[y] ) == -1 && ( abun[i] + abun[y] ) <= 28123 ) {
				num.push( abun[i] + abun[y] );
			}
		}
	}
	console.log( num );
...
function wordChange() {
	...
	for( var i = 2013456789; i <= 9876543210; i++ ) {
		...
		if( num.indexOf('0') != -1 && num.indexOf('1') != -1 && num.indexOf('2') != -1 &&
			num.indexOf('3') != -1 && num.indexOf('4') != -1 && num.indexOf('5') != -1 &&
			num.indexOf('6') != -1 && num.indexOf('7') != -1 && num.indexOf('8') != -1 &&
			num.indexOf('9') != -1 ) {
			count++;
			if( count > (999999 - 362880 - 362880) ) {
				return num;
			}
		}
...
function fib1000() {
	var first = new BigNumber(1);
	var second = new BigNumber(1);
	var next = new BigNumber();
	for(var i = 3; i; i++) {
		next = first.add(second);
		first = new BigNumber(second);
		second = new BigNumber(next);
		//document.write( next + ' = ' + i + '<br>' );
		if( (next + '').length >= 1000 ) {
			document.write(next);
			return i;
		}
			...
			for( var j = 3; j < num.length / 2 + 5; j++ ) {
				...
				if( period == period2  ) {
					if( j >= count ) {
						count = j;
						number = i;
						console.log( 'Период: ' + count + ' и число: ' + number );
						console.log( period );
					}
					break top;
				}
...
function money() {
	...
	for( var a = 1; a < Pi.length; a++ ) {
		coin = Pi[a];
		for( var b = 1; b <= sum; b++ ) {
			if( coin <= b ) {
				count[a][b] = count[a - 1][b] + count[a][b - coin];
			} else {
	...
	for( var i = 1; i <= count; i++ ) {
		for( var j = 1; j <= count; j++ ) {
			...
			if( num.indexOf('1') != -1 && num.indexOf('2') != -1 && num.indexOf('3') != -1 &&
				...
				if( total.indexOf(num2) == -1 ) {
					//total.push(num2);
				}
				if( total.indexOf(num1) == -1 ) {
					//total.push(num1);
				}
...
function circlePrime() {
	...
	for( var i = 0; i < arr.length; i++ ) {
		...
		var first = arr.indexOf( (num[1] || '') + (num[2] || '') + (num[3] || '') + (num[4] || '') + (num[5] || '') + (num[0] || '') );
		var second = arr.indexOf( (num[2] || '') + (num[3] || '') + (num[4] || '') + (num[5] || '') + (num[0] || '') + (num[1] || '') );
		var third = arr.indexOf( (num[3] || '') + (num[4] || '') + (num[5] || '') + (num[0] || '') + (num[1] || '') + (num[2] || '') );
		var fourth = arr.indexOf( (num[4] || '') + (num[5] || '') + (num[0] || '') + (num[1] || '') + (num[2] || '') + (num[3] || '') );
		var fifth = arr.indexOf( (num[5] || '') + (num[0] || '') + (num[1] || '') + (num[2] || '') + (num[3] || '') + (num[4] || '') );
